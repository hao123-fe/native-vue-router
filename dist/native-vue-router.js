...
(function (global, factory) {
	...
	(global.NativeVueRouter = factory());
...
var transitionName = css3Check('transition');
var transitionAttr = 'all 0.3s ease-in';
  ...
  methods: {
    ...
    initTouchEvent: function initTouchEvent() {
      ...
      rootEl.addEventListener('touchstart', function (e) {
        ...
        // 获取最顶部活性页面
        var i = self.$routeStack.length - 1;
        // 如果在首页则取消滑动后退
        if (i === 0) {
          return
        }
...
var START = createRoute('init', null, {
  path: '/'
});
...
var Link = {
  ...
  render: function render (h) {
    ...
    var ref = router.resolve(this.method, this.to, current, this.append);
    ...
    var compareTarget = location.path
      ? createRoute(this.method, null, location, null, router)
      : route;
...
function install (Vue) {
  ...
  Object.defineProperty(Vue.prototype, '$routeStack', {
    get: function get () { return this._routerRoot._routeStack}
  });
...
function createMatcher (
  ...
  function match (
    ...
    if (name) {
      ...
      if (!record) { return _createRoute(method, null, location) }
      ...
      if (record) {
        ...
        return _createRoute(method, record, location, redirectedFrom)
      }
      ...
      for (var i = 0; i < pathList.length; i++) {
        ...
        if (matchRoute(record$1.regex, location.path, location.params)) {
          return _createRoute(method, record$1, location, redirectedFrom)
        }
      }
    }
    // no match
    return _createRoute(method, null, location)
  }
  ...
  function redirect (
    ...
    var redirect = typeof originalRedirect === 'function'
        ? originalRedirect(createRoute(method, record, location, null, router))
        : originalRedirect;
  ...
  function alias (
    ...
    if (aliasedMatch) {
      ...
      return _createRoute(method, aliasedRecord, location)
    }
    return _createRoute(method, null, location)
  }
  ...
  function _createRoute (
    ...
    if (record && record.redirect) {
      return redirect(record, redirectedFrom || location, method)
    }
    if (record && record.matchAs) {
      return alias(record, location, record.matchAs, method)
    }
    return createRoute(method, record, location, redirectedFrom, router)
  }
...
function pushState (router, url, replace) {
  saveScrollPosition();
...
function replaceState (router, url) {
...
function immutableInit(config) {
  ...
  function flatMap(iterator) {
    ...
    for (index = 0; index < length; index++) {
      var iteratorResult = iterator(this$1[index], index, this$1);
  ...
  function without(remove) {
    ...
    for (var key in this$1) {
      if (this$1.hasOwnProperty(key) && remove(this$1[key], key) === false) {
        result[key] = this$1[key];
      }
    }
  ...
  function asMutableArray(opts) {
    ...
    if(opts && opts.deep) {
      for(i = 0, length = this.length; i < length; i++) {
        result.push(asDeepMutable(this$1[i]));
      }
    } else {
      for(i = 0, length = this.length; i < length; i++) {
        result.push(this$1[i]);
      }
    }
  ...
  function asObject(iterator) {
    ...
    for (index = 0; index < length; index++) {
      var pair  = iterator(this$1[index], index, this$1),
          key   = pair[0],
          value = pair[1];
      ...
      for (var index = 0, length = other.length; index < length; index++) {
        ...
        for (key in otherFromArray) {
          if (otherFromArray.hasOwnProperty(key)) {
            addToResult(result !== undefined ? result : this$1, otherFromArray, key);
          }
        }
      }
    }
    ...
    if(opts && opts.deep) {
      for (key in this$1) {
        if (this$1.hasOwnProperty(key)) {
          result[key] = asDeepMutable(this$1[key]);
        }
      }
    } else {
      for (key in this$1) {
...
History.prototype.transitionTo = function transitionTo (method,
  location, 
  onComplete, 
  onAbort) {
    var this$1 = this;
    ...
    try {
      hook(route, current, function (to) {
        if (to === false || isError(to)) {
          ...
          this$1.ensureURL(route.method, true);
          abort(to);
        } else if (
          typeof to === 'string' ||
          (typeof to === 'object' && (
...
var HTML5History = (function (History$$1) {
    ...
    window.addEventListener('popstate', function (event) {
      ...
      if (preHistory && state.key === preHistory.key) {
          ...
          if (state.key === nextHistory.key) {
              router.historyIndex++;
              this$1.onPopState('forward', state.path);
          } else {
  ...
  HTML5History.prototype.onPopState = function onPopState (method, location) {
    var current = this.current;
    if (method === 'back') {
  ...
  HTML5History.prototype.push = function push (location, onComplete, onAbort) {
    ...
    this.transitionTo('push', location, function (route) {
      pushState(this$1.router, cleanPath(this$1.base + route.fullPath));
  ...
  HTML5History.prototype.ensureURL = function ensureURL (method, push) {
    if (method === 'back') {
      return
    }
...
var HashHistory = (function (History$$1) {
  ...
  HashHistory.prototype.ensureURL = function ensureURL (method, push) {
...
var NativeVueRouter = function NativeVueRouter (options) {
...
NativeVueRouter.prototype.match = function match (
  method,
  raw,
  current,
  redirectedFrom
...
NativeVueRouter.prototype.clearInvalidRoute = function clearInvalidRoute () {
  ...
  while (this.routeStack[this.routeStack.length - 1].valid === false 
          ...
          this$1.routeStack.pop();
        }
};
NativeVueRouter.prototype.updateView = function updateView () {
  this.app._routeStack = seamlessImmutable.asMutable(this.routeStack);
};
NativeVueRouter.prototype.init = function init (app /* Vue component instance */) {
    var this$1 = this;
  ...
  history.listen(function (route) {
    ...
    this$1.apps.forEach(function (app) {
      ...
      if (method === 'push') {
        this$1.routeStack.push(nextRoute);
      } else if (method === 'replace') {
        this$1.routeStack[this$1.routeStack.length - 1].valid = false;
        this$1.routeStack.push(nextRoute);
      } else if (method === 'back') {
        // 先清空堆积的废页面,只有在back时最适合，其它时刻会用被删掉的dom替换新的dom
        this$1.clearInvalidRoute();

        var pageCount = this$1.routeStack.length;
        if (pageCount > 1) {
            this$1.routeStack[this$1.routeStack.length - 1].state = 'pop';
        } else {
            this$1.routeStack = [nextRoute];
        }
      }
...
NativeVueRouter.prototype.beforeEach = function beforeEach (fn) {
  return registerHook(this.beforeHooks, fn)
};

NativeVueRouter.prototype.beforeResolve = function beforeResolve (fn) {
  return registerHook(this.resolveHooks, fn)
};

NativeVueRouter.prototype.afterEach = function afterEach (fn) {
  return registerHook(this.afterHooks, fn)
};

NativeVueRouter.prototype.onReady = function onReady (cb, errorCb) {
  this.history.onReady(cb, errorCb);
};

NativeVueRouter.prototype.onError = function onError (errorCb) {
  this.history.onError(errorCb);
};

NativeVueRouter.prototype.push = function push (location, onComplete, onAbort) {
  this.history.push(location, onComplete, onAbort);
};

NativeVueRouter.prototype.replace = function replace (location, onComplete, onAbort) {
  this.history.replace(location, onComplete, onAbort);
};

NativeVueRouter.prototype.go = function go (n) {
  this.history.go(n);
};
...
NativeVueRouter.prototype.getMatchedComponents = function getMatchedComponents (to) {
  var route = to
    ? to.matched
      ? to
...
NativeVueRouter.prototype.resolve = function resolve (
  method,
  to,
  current,
  append
  ...
  var route = this.match(method, location, current);
...
NativeVueRouter.prototype.addRoutes = function addRoutes (routes) {
...
Object.defineProperties( NativeVueRouter.prototype, prototypeAccessors );
...
NativeVueRouter.version = '1.0.0';

if (inBrowser && window.Vue) {
  window.Vue.use(NativeVueRouter);
}

return NativeVueRouter;
